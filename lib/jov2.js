/* eslint-env node */
/* eslint max-len: [1, 90], curly: 0 */
/* eslint no-underscore-dangle: 0, no-plusplus: 0 */
...
function _ascii2utf8(buffer) {
  ...
  utf8 = new Buffer(buffer.length * 2);
  ...
  for (i = 0; i < buffer.length; i++) {
    ...
    if (c < 0x80) {
      utf8.writeUInt8(buffer.readUInt8(i, 1), j++);
    } else if (c < 0xc0) {
      utf8.writeUInt8(0xc2, j++);
      utf8.writeUInt8(buffer.readUInt8(i, 1), j++);
    } else {
      utf8.writeUInt8(0xc3, j++);
      utf8.writeUInt8(buffer.readUInt8(i, 1) - 0x40, j++);
    }
  }
  ...
  fs.access(db, fs.R_OK, function(error) {
    ...
    if (error) {
      ...
      if (error.code === 'EACCES')
        throw new Error('You don\'t have the rights to access this database!');
      else
        throw new Error('???');
    }
    ...
    fs.readFile(db, function(err, data) {
      ...
      if (byte0 !== 0 && byte0 !== 1 && byte0 !== 2 && byte0 !== 3)
        throw new Error('This database seems corrupted!');
...
module.exports = {
   ...
   * @returns {}          -,
   */
  getRecord: function(db, callback) {
    _readOv2(db, function(data) {
      callback(data);
    });
  }
};
